...
class AlertController extends Controller
{
    ...
    /*
    |--------------------------------------------------------------------------
    | Optional Warehouse NMS ID
    |--------------------------------------------------------------------------
    |
    | /master-alert-summary/dashboard
    |     -> Existing all-warehouse response
    |
    | /master-alert-summary/dashboard/31
    |     -> Only warehouse where nms_id = 31
    |
    */
    ...
    if ($warehouseNmsId !== null && $warehouseNmsId !== '') {
        ...
        if (! $warehouse) {
            return response()->json([
                ...
                'warehouseNmsId' => $warehouseNmsId,
            ], 404);
        }

        // Filter dashboard using stable warehouse code resolved from NMS ID.
        $validated['warehouseCode'] = $warehouse->warehouse_code;
    }

    return response()->json(
        $summaryService->fetchDashboard($validated)
    );
}
